...
var acceptedFiles = 'image/*,.psd,.pdf,.doc,.docx,.xls,.xlsx,.zip,.rar,.csv,.svg,.mp4,.wmv,.flv,.ogm';
var acceptedImage = 'image/*';
var filePath = '/images/admin/file-icons/';
var videoPath = '/images/admin/video-icons/';
            ...
            drop.on("success", function (file, response)
            {
                if ($(obj).hasClass('multi'))
                {
                    ...
                    if (response && response.link)
                    {
                        ...
                        {
                            //uploadUrl = "/" + language + "/" + controller + "/AddImage/aboutus/" + $('#propId').val() + "?image=" + response.link;
                            uploadUrl = "/" + controller + "/AddImage/aboutus/" + $('#propId').val() + "?image=" + response.link;
                        } else
                        {
                            console.log($('#propId').val());
                            //uploadUrl = "/" + language + "/" + controller + "/AddImage/" + $('#propId').val() + "?image=" + response.link;
                            uploadUrl = "/" + controller + "/AddImage/" + $('#propId').val() + "?image=" + response.link;
                        }
                        $.ajax({
                            type: "POST",
                            url: uploadUrl,
        ...
        function (i, obj)
        {
            ...
            drop.on("success", function (file, response)
            {
                ...
                if (response && response.link)
                {
                    ...
                    this.emit('thumbnail', file, ImageExist(filePath + ex + ".png") ? filePath + ex + ".png" : filePath + "file.png");
                    f.val(response.link);
    ...
    $('div.dropzone.video').each(
        ...
        function (i, obj) {
            ...
            var drop = new Dropzone(obj, {
                ...
                dictDefaultMessage: $(obj).find(".dz-clickable span").text(),
                removedfile: function (video) {
                    video.previewElement.remove();
                    f.val('');
                }
            });
            drop.on("sending", function (video, xhr, formData) {
                formData.append("prefix", $("#prefix").val())
                $(":submit").prop("disabled", true);
            });
            drop.on("queuecomplete", function () {
            ...
            drop.on("success", function (video, response) {
                if (this.files.length > 1) {
                    this.removeFile(this.files[0]);
                }
                if (response && response.error) {
                ...
                if (response && response.link) {
                    //this.emit('thumbnail', file, response.link);
                    var ex = response.link.split(".")[1]
                    this.emit('thumbnail', video, ImageExist(videoPath + ex + ".png") ? videoPath + ex + ".png" : filePath + "video.png");
                    f.val(response.link);
